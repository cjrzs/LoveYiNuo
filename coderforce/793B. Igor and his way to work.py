...
def bfs(startx, starty):
    ...
    while q:
        x, y, forward, cnt = q.popleft()
        # print(x, y, forward, cnt, q)
        if nums[x][y] == 'T':
            return True
        for i in range(4):
            a, b = dx[i] + x, dy[i] + y
            t = cnt
            if forward != -1 and forward != i:
                t = cnt + 1
            if t > 2 or a < 0 or a >= n or b < 0 or b >= m or vis[a][b] < t or nums[a][b] == '*':
                continue
            vis[a][b] = t
            q.append((a, b, i, t))
    return False
